...
- **授權**：**BSD 3-Clause**（`LICENSE`，Copyright (c) 2026, **Chih-Wei Su (Awaysu)** —— 本名 2026-08 補上，因為 Certum 開源憑證要能從公開紀錄把證件本名連到本專案；2026-08 補上——在那之前 repo 沒有授權條款，法律上等於保留所有權利，跟 README 的「歡迎自由修改」矛盾）。選 BSD-3 而非 MIT 是因為它的第 2 條明文要求**二進位散布也要在文件/隨附材料保留著作權聲明**，對應「請在你的關於視窗提及來源」；第 3 條擋掉冒用名義背書。內建的 LibRaw(LGPL 2.1，動態連結+附原始碼) 與 ExifTool(Artistic，獨立行程) 維持各自授權，不影響本專案採用 BSD。
...
| `--dlgshot <export\|settings\|progress\|gradient\|gradoverlay\|presets\|about\|fonts\|firstrun> <png>` | 截對話框（**folder 已於 2026-08 移除**——開啟資料夾改用系統 `FolderBrowserDialog`，系統模態視窗無法離屏截圖；progress=進度視窗、gradient=漸層 ribbon、gradoverlay=漸層覆疊手把、presets=編輯風格檔、about=關於、**fonts=字體大小**、**firstrun=第一次執行的語言選擇**）|
| 環境變數 `AWPR_UI_SCALE=2.0` | 強制介面縮放（字型一併補正），不必改系統顯示設定就能截各縮放的版面 |
| `--gputest <img> <report>` | **GPU 對照**：同一張 proxy＋全解析度，CPU 與 GPU 各算 15 組調整，量最大差／8-bit 差／耗時（見「GPU 加速」）|
...
- **滾輪 `e.Delta` 有縮放**（ThumbnailStrip / DarkScrollHost）：格子變大了，捲動量要跟著大，手感才一致。
...
- **`App/`** — `AppPaths`（支援副檔名、RAW_TEMP 忽略、三種快取檔命名、工具偵測；**2026-07 專案由 awPhotoRawEditor 改名為 AwayPhotoRawEditor**：`AppDataDir` 首次建立 `%AppData%\AwayPhotoRawEditor` 時會從舊名資料夾一次性複製設定/風格檔）、`AppSettings`（UseLibRaw / UseHighPrecisionRawPipeline / **UseGpu**(GPU 加速，預設 true；見「GPU 加速」) / **ShowColumnScrollBars**(顯示捲軸，預設 false) / **ShowHiddenPhotos**(預覽列「顯示全部」模式，預設 false=「不顯示隱藏」) / **InterfaceStyle**(`ClassicDark` 經典深色、`WarmPaper` 暖白相紙) / **UiScalePercent**(介面大小，0=自動依螢幕) / **FontSizes**(12 種字級，單位=100% 下的整數像素；見「高 DPI 縮放」) / **UiLanguage**(`TraditionalChinese`/`English`/`Japanese`/`Korean`/`SimplifiedChinese`/`German`/`French`/`Spanish`，enum 新值只能往後加)，存 `%AppData%\AwayPhotoRawEditor\settings.xml`；**`IsFirstRun`**＝啟動時 settings.xml 不存在，用來決定要不要跳語言選擇）、`Localization`/`L`（繁中 source text 作穩定 key，`Tr` 為 7 語 record(En/Ja/Ko/Hans/De/Fr/Es)，`T`/`F` 翻譯靜態與格式化文字，`Pick` 8 參數處理歧義詞，**`GuessFromSystem`** 依 `InstalledUICulture` 猜第一次執行的預選語言（認不得退回英文），`Apply` 遞迴翻譯 Control/SectionPanel/AdjustmentSlider/TopTab；風格檔內部名稱維持繁中 storage key、只翻譯顯示層；**德/法/西文字較長，固定寬度控制項的翻譯要挑短的**——已縮短 瀏覽/儲存至次資料夾/啟用浮水印/儲存設定(並開始轉存)/匯出目前照片 等）、**`AppVersion`（`Version = "v1.0.16"`；⚠️ 每次交付更新把第三位 +1 並同步 csproj `<Version>`，關於視窗顯示此值；`BuildTime` 讀 csproj AssemblyMetadata "BuildTime"＝編譯時間，關於視窗顯示）**
- **`App/UpdateCheck`（2026-08-24 起）** — 關於視窗「檢查更新」按的就是它。
  `GET api.php?action=check_update&app=awayphotoraweditor&platform=windows&version=<目前版本>`；
  API 全文見 GitHub private repo **`awaysu/software-web`** 的 `readme_for_program.txt`（本機沒有副本，要看就 clone）。
  - **版本比較交給伺服器的 `update_available`**，不在程式端自己比：readme 註明規則是 PHP `version_compare`，
    自己重寫一份遲早會分岔（`1.0.17-beta < 1.0.17` 這類後綴規則）。送出的版本帶不帶開頭的 `v` 都可以。
  - ⚠️ **更新說明不能讀回應裡的 `release_notes`**：那個欄位取版本歷史的第一筆，而本專案在網站上的歷史是
    **舊版在上**，所以它回的是 **v1.0.0** 的內容（2026-08-24 實測）。改用 `action=changelog&version=<最新版>`
    指名版本，與排序無關。**指名查不到就留空，不退回 `release_notes`**——網站還沒補該版歷史時，
    退路會在「最新版本 v1.0.17」底下印出 v1.0.0 的說明，比什麼都不顯示更糟。
  - 失敗（沒網路／逾時／回 HTML 錯誤頁）一律回 `null` 由呼叫端決定，readme 要求啟動時的自動檢查必須靜默略過。
  - 只做「查詢＋開下載頁」，**不自動下載安裝**：同時發行安裝檔與攜帶版 zip，程式無從知道使用者手上是哪一種。
...
- **`Diagnostics/`** — `SelfTest`（`[工具偵測]` 多印 GPU 狀態）、**`GpuParity`**（`--gputest`）、`GalleryForm`、`Trace`（僅診斷用，可保留）
...
### 預覽列縮圖套用調整（2026-08 起，v1.0.14）
縮圖不再是「永遠的原圖」，會呈現編輯後的樣子。
- **即時同步（2026-08-22 起，v1.0.16；使用者要求，推翻 v1.0.14 的「只在寫檔時重畫」）**：`OnAdjustmentChanged` 每次觸發都重啟 `_thumbLiveTimer`（200 ms debounce），到期後 `LiveRefreshCurrentThumbnail` 以**目前尚未存檔的** `_adj.Clone()` 在背景重畫目前這張的縮圖（含裁切/旋轉/漸層/修護——任何會經過 `OnAdjustmentChanged` 的操作）。`_thumbLiveVersion` 版本號讓晚到的舊算圖被丟掉；存檔時的 `RefreshThumbnail`（同一張）會停掉排隊中的即時重畫並把版本號 +1。**不寫檔**——XML 仍只在下列時機寫。批次同步的「其他」照片仍維持 flush 時才重畫（幾十張即時算太重）。
- **寫檔時機**：目前這張 → `SaveCurrentIfDirty`(切圖/匯出/關資料夾/關視窗)；批次/貼上設定/套用風格檔/重設 影響到的**其他**照片 → 當下就寫檔，所以立刻重畫；`RefreshStripKeepSelection`(隱藏/刪除後重建縮圖列，`SetItems` 會清掉所有圖) → 先同步塞回快取縮圖不閃空格、已編輯的再由背景蓋上去；開資料夾 → 併進既有的 `GenerateThumbnailCaches` 迴圈。
- **底圖＝現成的 `_thumb.jpg`**（相機內嵌預覽），不重生、不動全解碼。`RenderThumbnail` 走同一條 `ImageProcessor.Apply`（裁切/漸層/修護座標都是正規化的，縮圖尺寸算出來一致；**不帶 Watermark**，240×160 上只會是雜訊）。
- **⚠️ `RebaseWhiteBalance`**：RAW 縮圖的底圖是相機內嵌預覽、**相機白平衡已經烤在裡面**；主預覽的底圖 proxy 走 LibRaw 解碼且 **`use_camera_wb` 沒設定**，色溫滑桿才是唯一的白平衡來源。所以縮圖若照 `adj.Temperature` 原值算會**把白平衡套第二次**（鎢絲燈場景 3200K 的縮圖會整個偏藍）。改成套「相對拍攝時設定的偏移」：`5200 + (adj.Temperature - exif.ColorTemperature)`，只對 RAW 且 EXIF 有 as-shot WB 時做。非 RAW 的縮圖與 proxy 同樣走 WIC、底圖一致，不需要 rebase。
- **虛擬副本順便修好**：以前副本直接沿用來源縮圖，三個副本調成三種風格在預覽列長得一模一樣；現在依 `item.Key` 各套自己那份 XML（縮圖檔仍共用來源的 `_thumb.jpg`）。
- `AdjustmentXmlStore.LoadAll` 一次讀出 Adjustments/Exif/IsPlaceholder（`Load`/`LoadExif`/`IsDefaultPlaceholder` 各自會重新 parse 一次檔案，整個資料夾跑下來很浪費）。
...
## 色彩管線：處理版本（2026-08-21 起，v1.0.15）

v1.0.15 之前的三個問題（白平衡/曝光乘在 gamma 值上、K 值是黑體近似、「高精度」開關其實是 8-bit）已修，
採 **Lightroom 式「處理版本」**：舊照片永遠用舊算式、直到使用者主動升級。**舊版路徑以 11 組合成影像 hash
驗證過新舊 DLL 逐 bit 相同**（scratchpad `verify/`），改動管線時請維持這個保證。

### 版本機制
- `ImageAdjustments.PipelineVersion`：**`[XmlIgnore]` 的 public 欄位**（不是屬性）——反射式 ValueEquals/CopyFrom/ApplyDelta 只掃屬性，所以舊版照片沒動滑桿不會被當成「已編輯」、ResetAll 不會偷偷升級；MemberwiseClone 照樣帶著走。`CurrentPipelineVersion = 1`；`IsLegacyPipeline` = 0。
- 版本實際存在 `RawPipeDocument.PipelineVersion`（文件層）。舊 XML 沒有此元素 → 反序列化為 0；`new ImageAdjustments()`（沒 XML 的照片、批次 `?? new`）都是 1。
- **升級**：縮圖右鍵「升級處理版本」（`UpgradeSelectedPipeline`，確認對話框，多選可用）。`ConvertLegacyToLinear` 換算舊值讓畫面盡量接近：**曝光 ×2.22**（舊版乘在 709 gamma 值上，+1 其實是 +2.2 EV；漸層曝光同樣）、色溫保留「相對舊 as-shot（EXIF K 值）的偏移」套到新 as-shot（cam_mul 算出）上、Tint 疊加。狀態列顯示「· 舊版處理」提示。
- 曝光滑桿範圍隨版本：舊版 ±2（維持舊滑桿位置對應）、新版 ±5 真 EV（`BasicAdjustPanel.Bind` 在 base.Bind **之前**改範圍，否則值被舊範圍夾住）。
- 內建風格檔的曝光值（人像 0.2／柔和 0.3）以新版為準，套在舊版照片上會強 2.2 倍——可接受，不做版本分支。

### 新版（v1）算式 — `ImageProcessor.WhiteBalanceExposureLinear` + `ColorScience`
- 只有第 1+2 步（白平衡＋曝光）隨版本分支；色調曲線、飽和度、銳利度…全部維持在編碼域（當初就是在那裡調的）。漸層的**曝光**同樣走線性（`ApplyGradient(linearExposure)`），其他漸層參數不動。
- **轉換曲線＝LibRaw 預設 `gamm={0.45,4.5}`（BT.709 OETF）的反函數**，查表（4096/8192 格）不算次方；`Linearize`/`Encode` 輸入夾 0..1，亮部在這裡裁切（和舊版色調 LUT 裁切的位置一樣）。JPEG 是 sRGB、差在趾部幾個百分點，統一用一條。
- **白平衡矩陣**（有相機色彩資料時）：`M = rgb_cam · diag(mul_K / mul_ref) · rgb_cam⁻¹`，在**線性 sRGB** 上做、等價於相機空間的 von Kries。`mul_ref` 依 `ProcessContext.WhiteBalanceReference`：`Decode`＝pre_mul（LibRaw proxy/全解碼）、`AsShot`＝cam_mul（相機內嵌預覽＝縮圖）。**矩陣做亮度正規化**（中性灰 Y 維持 1），不然 6500→3200K 會暗 20%。沒有色彩資料（非 RAW／LibRaw 讀不到／四色感光元件）→ 退回黑體近似 `WhiteBalanceMultipliers`，但**在線性域乘**。
- **K 值定義**：沿黑體軌跡（Kang 2002，2000–12000K），Tint＝uv 平面上垂直軌跡的 Duv（`DuvPerTintUnit=0.0002`，±100≈±0.02 Duv≈舊版 ±30% 綠增益的強度；**+tint＝洋紅**）。`CamMulToKelvinTint`（cam_mul→K/Tint：as-shot、滴管）與 `KelvinTintToCamMul`（滑桿→增益）互為反函數，round-trip 誤差 <0.1%。**滑桿 K＝假設的光源色溫**（LR 慣例、舊版亦同）：設 3200K＝「現場是鎢絲燈」→ 加藍補償；低 K 畫面變冷。
- 相機色彩資料：`CameraColorInfo`（PreMul/CamMul 皆 G=1 正規化、RgbCam 3×3）存進 `ExifData.Camera`→ 隨 XML 快取。`LibRawInterop.ReadCameraColor` 用 C API `libraw_get_pre_mul/cam_mul/rgb_cam`（**只 open 不 unpack**，便宜）；rgb_cam 第 4 欄非 0（四色）→ null。`RawLoader.EnrichCameraColor` 在 `LoadPhoto`（EnsureDefault **之前**，它要靠這個種 as-shot）、`Exporter`、升級時補齊舊 XML 並存回。
- 滴管：`EstimateWhiteBalance` 新版＋有色彩資料 → `NeutralizingCamMul` 精確解；否則黑體搜尋（新版在線性值上、舊版在編碼值上——對應各自實際乘的東西）。

### 「高精度 RAW 管線」現在是真的 16-bit
- `Exporter.DecodeFull` 回傳 `FloatImageBuffer` 直接走 `Apply(FloatImageBuffer)`（以前在這裡 `ToBitmap()` 壓成 8-bit）。
- proxy 改存 **`.f16`**（ushort，`CacheManager.SaveHalf/LoadHalf`，LibRaw 16-bit 輸出無損、檔案 ≈35MB）；`ScaleFloat` 改 `ResizeFloatToMaxDim`（float 分離式面積平均），不再經 Bitmap 往返。舊 `.f32` 裝的是 8-bit 量化值，**換副檔名讓它自然失效**（DeleteCacheFiles 兩種都刪）。

### 仍然保留的近似（刻意不動）
- `no_auto_bright=0`（自動亮度開著）：逐張浮動的基準增益。關掉會改變所有照片基準亮度、且 PNG proxy 快取也得全部重生，不值得。
- LibRaw 解碼仍用 pre_mul（日光）而非 cam_mul：維持快取有效；白平衡矩陣把差額算回去。
- 單一光源的相機矩陣（LibRaw 只給一組）：K 值與 LR「同一級距」，跨相機一致度做不到 LR 的雙光源描述檔水準。
- 以上已用合成相機矩陣驗證數學結構；**實機 RAW（鎢絲燈／陰天場景）尚未驗證**，顏色幅度取決於真實 rgb_cam。

## GPU 加速（2026-08-21 起，v1.0.16）

使用者決定：**做 GPU 加速，偵測不到就退回 CPU**；一個 exe、不分「GPU 版／CPU 版」。
ComputeSharp 3.2（MIT，NuGet；shader 用 C# 寫、**編譯期**由 source generator 轉成 DXIL，
執行期只要 Direct3D 12，**不需要 dxcompiler.dll**，publish 目錄只多 `ComputeSharp.dll` + `ComputeSharp.Core.dll`）。
內顯可以跑（Intel Iris Xe / UHD 630 都有完整 D3D12 compute）；選 D3D12 而非 CUDA 就是為了這個。

### 架構（`Imaging/Gpu/`）
- **`GpuShaders.cs`**：5 個 compute shader，每個都是 `ImageProcessor` CPU 函式的逐行移植（同公式、同 LUT、同夾值位置）
  - `PixelStageShader`：步驟 1+2 白平衡/曝光（`PixelFlags` 三選一：LegacyWb／LinearMul／LinearMatrix）、3 色調 LUT、4 鮮豔度/飽和度、7 漸層（`GradientGpu[]` 依序疊）、10c 暗角，**融合成一個 kernel**，旗標決定做哪些
  - `BlurHShader` + `BlurVFinishShader`：方框模糊兩趟，垂直趟直接收尾（mode 0 Blend＝降噪/柔化、1 Unsharp＝銳利化），模糊結果不落地
  - `ResampleShader`：步驟 9 廣角變形（mode 0）與 10 裁切/拉直（mode 1 繞 (cx,cy) 旋轉、輸出原點 (ox,oy)），雙線性夾值同 `Sample`
  - `RotateShader`：90° 整數旋轉
  - 影像 buffer 一律 `ReadWriteBuffer<float4>`（RWStructuredBuffer 可讀可寫，讓一個 pass 的輸出直接餵下一個）；LUT／漸層參數是 `ReadOnlyBuffer`
- **`GpuPipeline.cs`**：裝置管理 + 兩條路徑
  - 裝置：`GraphicsDevice.QueryDevices(info => info.IsHardwareAccelerated)`，**不用 WARP**（軟體算圖比多執行緒 CPU 還慢），多顆挑 `DedicatedMemorySize` 最大的（筆電內顯＋獨顯選獨顯）。`DecodeTable/EncodeTable`（ColorScience 的兩張轉換曲線查表）開機上傳一次
  - **常駐路徑（resident）**：影像 `W×H×16` ≤ `ResidentCapBytes` 時整張上傳一次、所有階段在 GPU 上接力、最後下載一次。上限＝獨顯 `dedicated/2/3`、內顯 `shared/4/3`（同時最多三份整圖：目前／輸出／模糊暫存），再夾 2 GB。**Iris Xe 實測 1355 MB 上限，6600 萬像素（1.06 GB）的全解析度匯出整張放得進去**。失敗 → `ReportResidentFailure` 把上限降到該圖以下、改走分段（≤64 MB 的小圖失敗才算一次故障）
  - **分段路徑（banded）**：影像留在 CPU `FloatImageBuffer`，每段 ≤ 200 萬像素（32 MB）上傳→運算→下載。逐像素階段就地寫回、回傳「已完成列數」讓 CPU 補剩下的列；模糊階段每段帶 halo（兩個半徑之和）一起上傳，段內夾值只在影像邊緣才真的發生、結果與全圖相同；幾何階段來源整張上傳。連續失敗 3 次或 `DeviceLost` → `_broken`，整個程式只走 CPU
  - `Enabled` 由 `AppSettings.UseGpu` 同步（`Program.Main` 載入設定後、`SettingsForm` 套用時）；`StatusText` 給設定視窗／selftest 顯示
- **`ImageProcessor`**：步驟順序只寫一次在 `RunPipeline(IStageTarget)`，三種目標——`ResidentTarget`（全 GPU，失敗丟例外）、`BandedTarget`（每階段先試分段 GPU、沒做完的列用 CPU）；`ctx.ForceCpu` 讓 `--gputest` 拿 CPU 結果。**CPU 函式是參考實作，改算式兩邊要一起改**；逐像素 CPU 函式都帶 `(y0, y1)` 列範圍。`ResampleParams` 刻意用 double（CPU 的幾何算式要和改版前逐 bit 相同，送 shader 時才轉 float）。修護（Heal）維持 CPU：只碰幾個小圓，常駐路徑是 下載→CPU→上傳
- `ProcessContext.UsedGpu / GpuResident` 由管線回填；`MainForm` 狀態列尾巴顯示「· GPU 算圖」（標籤寬度 160→270，不然會被切掉）

### 驗證：`--gputest <img> <report>`（`Diagnostics/GpuParity.cs`）
同一張 proxy、15 組調整（舊版／v1 矩陣白平衡／色調曲線／降噪+銳利化／漸層／暗角／裁切+角度／變形／旋轉／綜合）＋全解析度綜合，
CPU 與 GPU 各跑一次比差異與耗時。**2026-08-21 Iris Xe 實測**：最大差 ≤ 7e-5（全解析度 2.9e-4，來自 10k 像素座標的 float 精度）、
沒有任何通道差超過 1/255、8-bit 輸出最多差 1 級且比例 ≤ 0.07%；耗時 proxy 級 2–9×（降噪+銳利化 697→74 ms、綜合 615→118 ms），
全解析度綜合 7.8 s → 2.0 s。**改 `ImageProcessor` 或 `GpuShaders` 後一定要重跑**，8-bit 差 ≥2 出現就是兩邊算式分岔了。

### 踩過的坑
- **從 PowerShell 啟動 `AwayPhotoRawEditor.exe` 會「拒絕存取」**（`Start-Process`／`&` 都是），`cmd /c ...exe` 或 `dotnet ...dll` 正常——跟程式無關，是這個 shell 環境的問題；診斷指令改用 cmd 啟動
- `GraphicsDevice` 沒有 `IsCacheCoherentUMA`（別憑印象用 API；`ComputeSharp.xml` 文件可 grep 簽章）
- shader 裡的 buffer 型別要和配置的一致：`ReadOnlyBuffer<float4>` 欄位收不了 `ReadWriteBuffer`，所以影像一律 RW
- 2D dispatch `For(W, rows)` 每維 thread group 上限 65535（×8 threads）；分段 200 萬像素遠在限制內，1D dispatch 反而會撞到（65535×64 ≈ 419 萬）
- 常駐路徑是「整張上傳、失敗退回」，所以它**只讀** `src`、絕不就地改；分段路徑才 `Clone()`
...
- **風格檔覆寫模型**：所有編輯/新增/恢復都在 `PresetEditorForm`（App 選單「編輯風格檔…」）。存全部欄位但 `ApplyCustom` 套用時只取 tonal/色彩/細節，**不含色溫／色調**（2026-08-23 使用者決定：風格檔一律不碰白平衡——v1.0.15 起照片色溫種成 as-shot，風格檔寫入絕對 K 值會把室內／陰天照片套成偏色。`ResetTonal` 不再重設 Temperature/Tint（`ResetBasicColorDetail` 仍會）、內建 風景/人像/柔和 的 5400–5650K 已拿掉、編輯視窗的色溫/色調滑桿**照常顯示**（使用者指定：不要因此改版面），改在藏起來的白平衡列位置放一行 `ColorPanel.ShowPresetWhiteBalanceNote()`「套用風格檔時維持照片目前的色溫／色調」；舊 presets.xml 裡殘留的 K 值被忽略）；內建風格檔的修改存成 PresetStore 覆寫（改回與內建預設完全相同時自動 `Remove` 覆寫）；自訂風格檔即 presets.xml 中非內建名稱的項目。**「預設時設定」(`DefaultName`, fullReset) 不在編輯清單、名稱不可用於自訂**。
