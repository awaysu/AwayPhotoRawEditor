...
| `--dlgshot <folder\|export\|settings\|progress\|gradient\|gradoverlay\|presets\|about\|fonts> <png>` | 截對話框（progress=進度視窗、gradient=漸層 ribbon、gradoverlay=漸層覆疊手把、presets=編輯風格檔、about=關於、**fonts=字體大小**）|
...
- **字級的正式單位是「100%（96 DPI）下的整數像素」**，換算 `pt = px × 0.75`：
  ...
  - **預設值（2026-08 使用者實機定案，比原始設計值大）**：Small **15**／Mono 11／Normal **15**／SectionTitle **16**／AboutBody 13／FolderGlyph 15／IconGlyph 16／ProgressTitle 16／DialogTitle 17／AboutTitle 19／Logo **22**／MenuGlyph 27。（原始設計值是 Small 11／Normal 12／SectionTitle 13／Logo 20，字偏小。）
