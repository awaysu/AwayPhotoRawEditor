...
## ⚠️ 色彩管線的已知限制（2026-08 查證，尚未決定要不要修）

**編輯區看到的不是 RAW，是 `RAW_TEMP/{檔名}.rawpipe.png`**（8-bit PNG，長邊上限 2560；高精度模式再多一份 `.rawpipe.f32` 32-bit 浮點）。不是 TIF。

### 1. 白平衡與曝光是在 gamma 空間乘的，不是線性光
`LibRawInterop.DecodeProcessed` 只設了 `output_color=1`(sRGB) 與 `output_bps`；**`libraw_set_gamma` 有宣告 P/Invoke 但從來沒被呼叫**，所以 LibRaw 用預設 gamma 曲線（dcraw 2.222/4.5）輸出，proxy 是**已經套過 gamma 的 sRGB 影像**。
而 `FloatImageBuffer.FromBitmap` 只做 `/255`，`ImageProcessor` 裡**沒有任何反 gamma 步驟**——`WhiteBalanceExposure` 直接把通道增益乘上 gamma 編碼值。
白平衡與曝光（`Math.Pow(2, EV)` 當乘數）本質都是線性光運算，放在 gamma 空間做，暗部與接近過曝處的行為和真正的 RAW 轉檔器不同。

### 2. K 值不是比色學意義上的色溫
`KelvinToRgb` 是 Tanner Helland 黑體近似公式，中性點寫死 5200K，**完全沒用到相機色彩矩陣或校正光源**。這裡的 5600K ≠ Lightroom/Capture One 對同一檔案的 5600K。是一套自洽的刻度，不是通用刻度。

### 3. LibRaw 沒開 camera WB
`use_camera_wb` 未設定 → LibRaw 用色彩矩陣的日光乘數；拍攝時白平衡另外由 `EnsureDefault` 從 EXIF 種進 `adj.Temperature`。設計本身自洽，但這正是縮圖必須 `RebaseWhiteBalance` 的原因（見「預覽列縮圖套用調整」）。

### 4. `no_auto_bright = 0` ＝自動亮度**開著**
LibRaw 依直方圖決定基準曝光，所以基準曝光是逐張浮動的。預覽與匯出走同一條解碼，故兩者一致。

### 預覽與匯出是否一致？一致。
`Exporter` 重新全解碼原始 RAW（`DecodeFullFloat`/`DecodeFullBitmap`）再跑同一條 `ImageProcessor.Apply`。差別只在**解析度**(2560 vs 全尺寸)與**位元深度**(非高精度模式下 proxy 是 8-bit)——預覽看到的過曝，匯出時可能還救得回來，反之亦然。
**結論：預覽 vs 匯出＝一致；本 App 的白平衡 vs 真正的 RAW 轉檔器＝確實不同。**

### 要修的話（三條路，都會改變所有現有照片的算圖結果）
存好的調整 XML 會算出不一樣的畫面，等於渲染基準換掉，屬於破壞性變更：
1. `libraw_set_gamma(lr, 0, 1.0)` + `(lr, 1, 1.0)` 拿線性解碼，WB/曝光在線性光做完，管線最後才套顯示 gamma。最正確、影響也最大
2. 只在管線內部反 gamma → WB/曝光 → 再套回去。改動範圍小一點，結果一樣會變
3. 從 LibRaw 撈真正的 `cam_mul`/`pre_mul` 推 Kelvin，讓數字有意義
