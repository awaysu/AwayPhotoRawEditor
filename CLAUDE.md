...
縮圖不再是「永遠的原圖」，會呈現編輯後的樣子。**刻意不做即時同步**：拖滑桿/拖裁切框時縮圖不動，
編輯當下的回饋由主預覽負責；縮圖只在**調整寫進磁碟**的時機重畫。
- **時機**：目前這張 → `SaveCurrentIfDirty`(切圖/匯出/關資料夾/關視窗)；批次/貼上設定/套用風格檔/重設 影響到的**其他**照片 → 當下就寫檔，所以立刻重畫；`RefreshStripKeepSelection`(隱藏/刪除後重建縮圖列，`SetItems` 會清掉所有圖) → 先同步塞回快取縮圖不閃空格、已編輯的再由背景蓋上去；開資料夾 → 併進既有的 `GenerateThumbnailCaches` 迴圈。
